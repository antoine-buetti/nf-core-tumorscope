...
from cuml.ensemble import RandomForestClassifier # inputs must be CuPy arrays or cuDF DataFrames.
...
def normalize_grayscale(image, low_perc=2, high_perc=98):
    """Normalize grayscale image using percentile clipping."""
    image_cp = cp.asarray(image)  # <-- ensure GPU array
    low = cp.percentile(image_cp, low_perc)
    high = cp.percentile(image_cp, high_perc)
    clipped = cp.clip(image_cp, low, high)
    normalized = (clipped - low) / (high - low)
    return (normalized * 255).astype(cp.uint8).get()  # return NumPy array to stay compatible
...
def main():
    ...
    feature_function = functools.partial(
        skimage.feature.multiscale_basic_features, # CPU-only, maybe use RAPIDS cuCIM (cucim.skimage) which provides GPU-accelerated equivalents of many skimage functions.
        intensity=True,
        edges=True,
        texture=True,
        sigma_min=1,
    ...
    while i < nb_frames:
        ...
        normalized_red = normalize_grayscale(red_frame)   # returns NumPy
        ...
        # features = feature_function(cp.asarray(normalized_red))  # ensure CuPy
    ...
    # X_train = cp.concatenate(X_train, axis=0)
    # y_train = cp.concatenate(y_train, axis=0)
    ...
    for i in tqdm(range(nb_frames)):
        red_frame = image[i, red_channel, :, :]
        test_img = normalize_grayscale(red_frame)         # NumPy

        features = feature_function(test_img)
        # features = feature_function(cp.asarray(test_img)) # CuPy

        h, w = test_img.shape

        # features_flat = features.reshape(-1, features.shape[-1])
        # pred_flat = classifier.predict(features_flat)
        features_flat = cp.asarray(features_flat)
        pred_flat = classifier.predict(features_flat).get()  # back to NumPy for reshaping
